...
from rhyme.models import Rhyme, Word
from rhyme.worker.settings import SAVETXT_2, SAVETXT_3
from rhyme.worker.rhyme_index import RhymeIndex
...
class Ext:
    ...
    def _get_words(self, integ):
        ...
        query = Word.objects.filter(rhyme = integ)
        if len(query) < 2:
            return
